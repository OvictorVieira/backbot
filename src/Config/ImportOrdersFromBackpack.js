...
class ImportOrdersFromBackpack {
  ...
  static async importActiveOrders(botId, config) {
    try {
      ...
      console.log(
        `📊 [IMPORT_ORDERS] Encontradas ${activeOrders.length} ordens ativas na Backpack`
      );
      ...
      for (const order of activeOrders) {
        try {
          ...
          console.error(
            `❌ [IMPORT_ORDERS] Erro ao importar ordem ${order.symbol}:`,
            orderError.message
          );
        }
      }
      ...
      console.log(
        `🎉 [IMPORT_ORDERS] Importação concluída: ${importedCount} ordens importadas para Bot ${botId}`
      );
